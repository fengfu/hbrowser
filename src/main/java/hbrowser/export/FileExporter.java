package hbrowser.export;

import hbrowser.data.ColumnQualifier;
import hbrowser.data.DataCell;
import hbrowser.data.DataRow;
import hbrowser.format.CharacterDelimitedFormatter;
import hbrowser.format.Formatter;
...
//        Formatter formatter = new CharacterDelimitedFormatter(this.delimiter);
        ...
        for (ColumnQualifier column : columns) {
            ...
            if (cell != null) {
                dataLength += cell.getValue().length();
            }
            columnCount++;
        }

        StringBuilder sb = new StringBuilder(dataLength+columnCount);
